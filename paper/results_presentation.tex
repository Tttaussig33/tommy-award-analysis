...
% ==================================================================
\section{Sample game data}
% ==================================================================
Table~\ref{tab:sample-game-mar6-2026} lists one Celtics game from the
hustle table (2026-03-06, game ID 22500908 in the source notebook export). The
\textbf{Tommy winner} column is the per-game label~\texttt{y} (1 = Tommy
recipient for that game). \textbf{Net rtg.} is the on/off proxy
\texttt{plusMinusPoints}, shown as a whole number as in the notebook.

\begin{table}[H]
    \centering
    \caption{Sample box-style rows for 2026-03-06 (Boston).}
    \label{tab:sample-game-mar6-2026}
    ...
    \begin{tabular}{@{}llcrrrrrrrr@{}}
        \toprule
        \textbf{Date} & \textbf{Player} & \textbf{Tommy win.} & \textbf{Min} & \textbf{PTS} & \textbf{REB} & \textbf{AST} & \textbf{OREB} & \textbf{STL} & \textbf{BLK} & \textbf{Net rtg.} \\
        \midrule
        2026-03-06 & Payton Pritchard     & 0 & 33.8 & 18 &  7 & 6 & 2 & 0 & 2 & 19 \\
        2026-03-06 & Jaylen Brown         & 0 & 33.3 & 24 &  7 & 7 & 3 & 2 & 0 & 20 \\
        2026-03-06 & Derrick White        & 0 & 31.1 & 20 &  4 & 4 & 1 & 2 & 2 & 23 \\
        2026-03-06 & Jayson Tatum         & 1 & 27.2 & 15 & 12 & 7 & 1 & 1 & 0 & 20 \\
        2026-03-06 & Neemias Queta        & 0 & 25.6 & 16 & 15 & 0 & 3 & 1 & 1 & 16 \\
        2026-03-06 & Sam Hauser           & 0 & 22.5 &  6 &  5 & 0 & 1 & 0 & 2 &  0 \\
        2026-03-06 & Baylor Scheierman    & 0 & 20.8 &  5 &  2 & 2 & 0 & 0 & 0 &  0 \\
        2026-03-06 & Hugo Gonz\'alez      & 0 & 18.9 &  6 &  2 & 2 & 0 & 0 & 1 &  2 \\
        2026-03-06 & Luka Garza           & 0 & 17.9 & 10 &  3 & 1 & 3 & 0 & 2 &  9 \\
        2026-03-06 & Ron Harper Jr.       & 0 &  4.3 &  0 &  1 & 0 & 0 & 0 & 0 & -4 \\
        2026-03-06 & Amari Williams       & 0 &  2.7 &  0 &  0 & 0 & 0 & 0 & 0 & -4 \\
        2026-03-06 & Nikola Vu\v{c}evi\'c & 0 &  1.8 &  0 &  0 & 0 & 0 & 0 & 0 & -1 \\
        \bottomrule
    \end{tabular}
\end{table}
...
2024--25 and 2025--26 seasons, sorted by Top-3 game accuracy from
highest to lowest (ties at four decimals broken by lower log loss).

\begin{table}[H]
    \centering
    \caption{All model results on the 2024--25 and 2025--26 test seasons, ranked by Top-3 game accuracy.}
    \label{tab:results}
...
Table~\ref{tab:results-top3} presents a condensed view with Top-3 game
accuracy for each model (same ranking as Table~\ref{tab:results}).

\begin{table}[H]
    \centering
    \caption{Model Top-3 game accuracy on the 2024--25 and 2025--26 test seasons.}
    \label{tab:results-top3}
    \renewcommand{\arraystretch}{1.25}
    \rowcolors{2}{lightgray}{white}
    \begin{tabular}{l S}
        \toprule
        \textbf{Model} & {\textbf{Top-3 Acc.}} \\
        \midrule
        Random forest        & 0.7853 \\
        Elastic-net logistic & 0.7485 \\
        Lasso logistic       & 0.7423 \\
        XGBoost              & 0.7362 \\
        Ridge logistic       & 0.7362 \\
...
\begin{itemize}
    ...
    \item To add a new model's row, copy any existing row in
          Table~\ref{tab:results} and replace the numbers (keep rows sorted
          by Top-3 accuracy, then lower log loss as a tie-break).
...
% ==================================================================
\section{Predicted Tommy wins per 60 minutes (full ranking)}
% ==================================================================
Table~\ref{tab:per60-full} lists every player in the cross-team prediction
export who logged at least \textbf{300} total minutes across
\texttt{predictions/*\_player\_game\_enriched.csv} (2024--25 and 2025--26),
merged with predicted win totals from
\texttt{predictions/*\_predicted\_tommy\_counts\_combined.csv}.
The rate is $60 \times (\text{predicted wins}) / (\text{total minutes})$.
Rows are sorted by per-60 rate (highest first). To refresh the table body
and the top-10 bar chart (Figure~\ref{fig:per60-top10}) after updating prediction CSVs, run
\texttt{python3 scripts/build\_results\_per60\_table\_tex.py}. Unicode names in the
fragment assume UTF-8 \texttt{pdflatex} or use \texttt{xelatex}/\texttt{lualatex}
...
\begin{longtable}{@{}r l l r r r@{}}
    \caption{Full league-wide ranking: predicted Tommy-style wins per 60 minutes (minimum 300 minutes).}
    \label{tab:per60-full}\\
    \toprule
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\textwidth]{results_per60_top10.png}
    \caption{Top ten players by predicted Tommy Award wins per 60 minutes (same data as the head of Table~\ref{tab:per60-full}); bar colors match franchise primaries from the build script.}
    \label{fig:per60-top10}
\end{figure}
