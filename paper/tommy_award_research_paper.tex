...
By combining net rating and usage rate, I engineered a feature called impact efficiency. The equation for impact efficiency is
\[
...
This could be grabbing rebounds, getting steals, things that aren't scoring, but are still positively impacting their team. This player archetype is what we are seeking with the Tommy Award.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{@{}lp{8.8cm}@{}}
    ...
    \texttt{stocks} & Steals plus blocks. \\
    \texttt{hustle\_proxy} & Composite hustle signal built from tracked hustle inputs. \\
...
Table~\ref{tab:fi} lists all feature importances for the fitted random forest, in the same order as \texttt{display(feature\_importance\_df)} in \texttt{notebooks/random\_forest\_classifier.ipynb} (sorted descending). Values are regenerated by \texttt{python3 scripts/export\_rf\_feature\_importances\_tex.py}. Figure~\ref{fig:fi} shows the top 10 for readability.

{\footnotesize
\setlength{\tabcolsep}{5pt}
\renewcommand{\arraystretch}{1.1}
\begin{longtable}{@{}r l r@{}}
  \caption{Random Forest: all feature importances (39 numeric features).}\label{tab:fi}\\
  \toprule
  \textbf{Rank} & \textbf{Feature} & \textbf{Importance} \\
  \endfirsthead
  \multicolumn{3}{c}{\textit{Table \ref{tab:fi} continued}}\\
  \toprule
  \textbf{Rank} & \textbf{Feature} & \textbf{Importance} \\
  \endhead
  \midrule
  ...
  \bottomrule
  \endlastfoot
  \input{results_rf_feature_importance_body.tex}
\end{longtable}
}
...
\section{References}
\begin{enumerate}
  \item Deshpande, S. K., \& Jensen, S. T. (2016). \textit{Estimating an NBA Player's Impact on His Team's Chances of Winning.} Journal of Quantitative Analysis in Sports.
  \item Houde, M. (2021). \textit{Predicting the outcome of NBA games} [Honors thesis, Bryant University].
  \item James, G., Witten, D., Hastie, T., Tibshirani, R., \& Taylor, J. E. (2023). \textit{An introduction to statistical learning: With applications in Python.} Springer.
  \item Kuehn, J. (2016). \textit{Accounting for complementary skill sets when evaluating NBA players' values to a specific team.} MIT Sloan Sports Analytics Conference.
  \item Sarlis, V., Gerakas, D., \& Tjortjis, C. (2024). \textit{A data science and sports analytics approach to decode clutch dynamics in the last minutes of NBA games.} Machine Learning and Knowledge Extraction.
  \item Sill, J. (2010). \textit{Improved NBA adjusted +/- using regularization and out-of-sample testing.} MIT Sloan Sports Analytics Conference.
  \item Williams, B., Schliep, E. M., Fosdick, B., \& Elmore, R. (2024). \textit{Expected points above average: A novel NBA player metric based on Bayesian hierarchical modeling.} arXiv.
\end{enumerate}
