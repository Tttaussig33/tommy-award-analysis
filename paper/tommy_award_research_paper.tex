...
Table~\ref{tab:fi} lists all feature importances for the fitted random forest, in the same order as \texttt{display(feature\_importance\_df)} in \texttt{notebooks/random\_forest\_classifier.ipynb} (sorted descending). Values are regenerated by \texttt{python3 scripts/export\_rf\_feature\_importances\_tex.py}.
